...
import { TUsersResponse as TUsers } from "../../types/Users";
type NewNoteFormProps = {
  users: TUsers[];
};
const NewNoteForm = ({ users }: NewNoteFormProps) => {
  console.log("users NewNoteForm: ", users);
  ...
  if (isError && error) {
    if ("status" in error) {
      ...
      errContent = <p className={errClass}>{errMsg}</p>;
    } else {
      errContent = <p className={errClass}>{error.message}</p>;
    }
  }
