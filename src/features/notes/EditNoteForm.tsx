...
import { TNotesResponse } from "../../types/Notes";
import { TUsersResponse } from "../../types/Users";
...
const EditNoteForm = ({ note, users }: EditNoteFormProps) => {
  const [
    updateNote,
    { isLoading, isSuccess, isError: isUpdateError, error: updateError },
  ] = useUpdateNoteMutation();

  const [
    deleteNote,
    { isSuccess: isDelSuccess, isError: isDeleteError, error: deleteError },
  ...
  if (deleteError) {
    if ("status" in deleteError) {
      ...
      errorDelete = deleteError.message;
    }
  }

  const errContent = (errorUpdate || errorDelete) ?? "";
